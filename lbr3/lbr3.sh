...
do
    ps[i]=$((i+1))
    read -p "Enter the brust time & priority for process P$((i+1)): " bt[i] pri[i]
    ((i++))
done
...
echo -e "\nProcess \t Brust Time  \tPriority  \t Watting Time \t Turnarround Time"
i=0
while [ $i -lt $process ]
do
    totalwt=$((totalwt+wt[i]))
    totaltat=$((totaltat+tat[i]))
    echo -e "  P${ps[i]} \t\t   ${bt[i]} \t\t  ${pri[i]} \t\t ${wt[i]} \t\t   ${tat[i]}"
    ((i++))
done

echo -e "\nAverage waiting Time ---> $((totalwt/process))"
